...
export class AddOutgoingPage {
  ...
  constructor(
    ...
    if(income) {
      ...
      this.form.controls.descricao.setValue(income.descricao);
  ...
  add(form: FormGroup) {
    ...
    this.userProvider.getCurrentUser().subscribe(({ email }) => {
      this.outgoingProvider.add(email, income, this.id).then(() => {
        this.loading = false;
  ...
  delete() {
    this.deleting = true;
    this.userProvider.getCurrentUser().subscribe(({ email }) => {
      this.outgoingProvider.delete(email, this.id).then(() => {
        this.deleting = false;
        this.navParams.get('reload')();
        this.navCtrl.pop();
      });
